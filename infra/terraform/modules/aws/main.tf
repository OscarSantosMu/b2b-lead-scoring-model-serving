...
# Grant ECS task execution role access to ECR
...
# Grant task role access to SageMaker if needed
    ...
    Statement = [
      {
        ...
        Resource = var.sagemaker_endpoint_name != "" ? "arn:aws:sagemaker:${var.aws_region}:*:endpoint/${var.sagemaker_endpoint_name}" : "*"
      }
    ]
  })
}
...
resource "aws_cloudwatch_metric_alarm" "high_cpu" {
  ...
  statistic           = "Average"
  threshold           = 80
  alarm_description   = "Alert when CPU usage is high"
